...
for(const selector of ['.pd-toggle','.pd-toggle-copy','.pd-toggle-meta','.pd-collapsed','.pd-enhanced[data-pd-state="empty"]','.pd-enhanced[data-pd-state="locked"]']){
  assert(css.includes(selector),`missing progressive-disclosure selector ${selector}`);
}
for(const token of ['--fl-border-default','--fl-surface-4','--fl-surface-2','--fl-text-primary','--fl-text-muted','--fl-warning-border','--fl-focus-ring']){
  assert(css.includes(`var(${token})`),`progressive disclosure should consume theme token ${token}`);
}
...
assert(sw.includes("frontier-lab-v14"),'Item 13.5 should advance the offline cache version');

console.log(JSON.stringify({progressiveDisclosureStatic:'pass',cache:'frontier-lab-v14'},null,2));
