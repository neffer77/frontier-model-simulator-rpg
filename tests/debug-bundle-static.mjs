...
for(const marker of [
  'frontierCreateDebugBundle','frontierDebugBundleSummary','frontierDownloadDebugBundle','frontierCopyDebugSummary','frontierOpenDiagnostics','frontierCloseDiagnostics','frontierDebugSanitize',
  "item:'P5.0.3'",'bundleId','actionTrail','reproduction','serviceWorker','cacheStorage','performanceSnapshot','storageSnapshot','domContext','[REDACTED]',
  "diagnostics.open","diagnostics.bundle","runtime.debug-bundle.ready"
])assert(runtime.includes(marker),`debug runtime missing contract ${marker}`);
...
const adapters=html.indexOf('command-adapters.js');
const debug=html.indexOf('debug-bundle.js');
assert(adapters>=0&&debug>adapters,'debug bundle must load after command adapters');
assert(html.includes('debug-bundle.css'),'debug diagnostics stylesheet missing from runtime');
assert(sw.includes("CACHE='frontier-lab-v30'"),'P5.0.3 must advance PWA cache to v30');
...
const gate=policy.gates.find(g=>g.id==='debug-bundle');
assert(gate,'release policy missing debug-bundle gate');
assert.equal(gate.severity,'blocker','debug-bundle must be release blocking');
assert.equal(gate.script,'test:debug-bundle','debug-bundle gate must invoke canonical browser test');
assert.equal(gate.evidence,'artifacts/debug-bundle/report.json','debug-bundle evidence path drifted');
...
console.log(JSON.stringify({debugBundleStatic:'pass',schemaVersion:1,pwaCache:'frontier-lab-v30',releaseBlocker:true},null,2));
