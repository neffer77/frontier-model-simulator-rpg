...
const report={
  ...
  missingCaptures:[],
  pageErrors:[],
  manualExclusions:inventory.specialCaptures.filter(x=>x.manual).map(x=>x.id)
};
...
const candidate={
  ...
  item:'13.14',
  status:'active',
  description:'Deterministic SHA-256 baselines for Item 13.14 full-page Playwright screenshots.',
  inventoryVersion:inventory.version,
  responsiveMatrixVersion:matrix.version,
  ...
  capturePolicy:{
    ...
    deterministicRuntime:'UTC timezone, en-US locale, dark color scheme, reduced motion, fixed Date and seeded Math.random',
...
function pngDimensions(buffer){
  ...
  if(buffer.length<24||buffer.subarray(0,8).toString('hex')!==signature)throw new Error('Screenshot buffer is not a PNG');
...
async function dismissStory(page){for(let i=0;i<12;i++){const overlay=page.locator('.story-overlay');if(!(await overlay.count()))break;const button=overlay.locator('button.primary').last();if(await button.count())await button.click();else{const any=overlay.locator('button').last();if(!(await any.count()))break;await any.click()}await settle(page,35)}}
async function goHome(page){await page.evaluate(()=>{if(typeof window.gameplayGoHome==='function')window.gameplayGoHome();else{state.view='company';save();render()}});await settle(page,70);await dismissStory(page);await sync(page)}
async function graduate(page){await page.evaluate(()=>{state.campaign||={version:1};state.campaign.graduated=true;state.campaign.companyPriority=state.campaign.companyPriority||'research';state.campaign.modelReviewed=true;save();render()});await settle(page,90);await sync(page)}
...
async function capture(page,viewport,id,label,stateClasses,extra={}){
  ...
  const entry={sha256:digest,width:dims.width,height:dims.height,bytes:buffer.length,label,stateClasses,kind:extra.kind||'special',screenId:extra.screenId||null};
  candidate.captures[key]=entry;
  const expected=baseline.captures?.[key]||null;
  let status='match';
  ...
  if(updateMode||status!=='match'){
    const dir=path.join(outRoot,status==='match'?'current':'changed',viewport.id);fs.mkdirSync(dir,{recursive:true});
    fs.writeFileSync(path.join(dir,`${slug(id)}.png`),buffer);
  }
  return row;
}
...
for(const viewport of matrix.viewports){
  ...
  await dismissStory(page);await goHome(page);
  ...
  if(hasMenu){await page.evaluate(()=>window.gameplayToggleMenu());await settle(page,50);await sync(page);const moreLocked=special('more-locked');await capture(page,viewport,'more-locked',moreLocked.label,moreLocked.requiredStates);await page.evaluate(()=>window.gameplayCloseMenu?.());await settle(page,35)}
  else recordProblem('missingCaptures',{key:expectedKey(viewport.id,'more-locked'),reason:'gameplayToggleMenu unavailable'});

  await page.evaluate(()=>window.gameplayGoTrain?.());await settle(page,180);await sync(page);const trainingEmpty=special('training-empty');await capture(page,viewport,'training-empty',trainingEmpty.label,trainingEmpty.requiredStates);
  await goHome(page);await graduate(page);
  if(hasMenu){await page.evaluate(()=>window.gameplayToggleMenu());await settle(page,50);await sync(page);const moreUnlocked=special('more-unlocked');await capture(page,viewport,'more-unlocked',moreUnlocked.label,moreUnlocked.requiredStates);await page.evaluate(()=>window.gameplayCloseMenu?.());await settle(page,35)}
  ...
  for(const screen of inventory.screens){
    ...
    await settle(page,90);await dismissStory(page);await sync(page);
  ...
  if(await page.evaluate(()=>typeof window.showExplain==='function')){await page.evaluate(()=>showExplain('FLOPS'));await settle(page,60);await sync(page);const explainer=special('technical-explainer');await capture(page,viewport,'technical-explainer',explainer.label,explainer.requiredStates);await page.evaluate(()=>{const root=document.getElementById('modalRoot');if(root)root.innerHTML=''});await settle(page,25)}
  else recordProblem('missingCaptures',{key:expectedKey(viewport.id,'technical-explainer'),reason:'showExplain unavailable'});

  if(await page.evaluate(()=>typeof window.gameFeelMilestone==='function')){await page.evaluate(()=>window.gameFeelMilestone('VISUAL QA','Representative milestone surface for the Item 13 screenshot baseline.'));await settle(page,60);await sync(page);const milestone=special('milestone');await capture(page,viewport,'milestone',milestone.label,milestone.requiredStates);await page.evaluate(()=>document.querySelector('.feel-milestone')?.remove());await settle(page,25)}
  else recordProblem('missingCaptures',{key:expectedKey(viewport.id,'milestone'),reason:'gameFeelMilestone unavailable'});

  if(await page.evaluate(()=>typeof window.campaignOpenPriority==='function')){await page.evaluate(()=>window.campaignOpenPriority());await settle(page,60);await sync(page);const priority=special('company-priority');await capture(page,viewport,'company-priority',priority.label,priority.requiredStates);await page.evaluate(()=>document.querySelector('.campaign-priority')?.remove());await settle(page,25)}
  else recordProblem('missingCaptures',{key:expectedKey(viewport.id,'company-priority'),reason:'campaignOpenPriority unavailable'});

  if(await page.evaluate(()=>typeof window.realismOpen==='function')){await page.evaluate(()=>window.realismOpen());await settle(page,80);await sync(page);const realism=special('realism-audit');await capture(page,viewport,'realism-audit',realism.label,realism.requiredStates)}
  else recordProblem('missingCaptures',{key:expectedKey(viewport.id,'realism-audit'),reason:'realismOpen unavailable'});

  await goHome(page);
  if(await page.evaluate(()=>typeof window.replayOpen==='function')){await page.evaluate(()=>window.replayOpen());await settle(page,80);await sync(page);const archive=special('run-archive');await capture(page,viewport,'run-archive',archive.label,archive.requiredStates)}
...
const expectedKeys=[];
for(const viewport of matrix.viewports){for(const screen of inventory.screens)expectedKeys.push(expectedKey(viewport.id,`route-${screen.id}`));for(const item of autoSpecials)expectedKeys.push(expectedKey(viewport.id,item.id))}
...
const baselineKeys=new Set(Object.keys(baseline.captures||{}));const expectedKeySet=new Set(expectedKeys);
for(const key of baselineKeys)if(!expectedKeySet.has(key))recordProblem('extraBaseline',{key});
...
for(const row of report.captures)lines.push(`| ${row.viewport} | ${row.id} | ${row.status} | ${row.width}×${row.height} | \`${row.sha256.slice(0,16)}…\` |`);
...
const baselineInactive=baseline.status!=='active'&&!updateMode;
const regressions=report.mismatches.length||report.missingBaseline.length||report.extraBaseline.length;
