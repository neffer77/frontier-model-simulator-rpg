...
assert(focusedWorkflow.includes('FrontierOS Company evidence'));assert(focusedWorkflow.includes('artifacts/company-frontieros'));assert(focusedWorkflow.includes('test:company-os-static'));assert(focusedWorkflow.includes('test:company-os'));
assert(browserWorkflow.includes('npm run test:rc'),'cross-device QA must execute the canonical release policy containing the Company blocker');
console.log(JSON.stringify({companyStatic:'pass',policyVersion:policy.version,cache:`v${cache[1]}`,views:6,actionBoundary:'frontierCompanyDispatch',deliveryAssets:4,releaseBlocking:true},null,2));
