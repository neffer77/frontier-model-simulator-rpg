...
for(const id of ['mail','pager','training','evals','model-lab','terminal','data','team','projects','company','finance','knowledge','code-lab','artifacts','settings'])assert(registry.includes(`id:'${id}'`),`canonical app missing: ${id}`);
for(const [id,command] of [['knowledge','knowledge.open'],['code-lab','code-lab.open'],['artifacts','artifacts.open']]){assert(registry.includes(`id:'${id}'`)&&registry.includes(`command:'${command}'`),`${id} must be command-owned`);assert(adapters.includes(`frontierRegisterCommand('${command}'`),`${command} registration missing`)}
...
const cacheMatch=sw.match(/CACHE='frontier-lab-v(\d+)'/);assert(cacheMatch,'PWA cache version missing');assert(Number(cacheMatch[1])>=47,`FrontierOS requires PWA cache v47 or newer; found v${cacheMatch?.[1]}`);
assert(sw.includes('./frontier-app-registry.js')&&sw.includes('./command-adapters.js'),'PWA must cache registry + command adapters');
...
console.log(JSON.stringify({appRegistryStatic:'pass',schemaVersion:1,apps:15,plannedSemantics:true,releaseBlocker:true,cache:`v${cacheMatch[1]}`},null,2));
