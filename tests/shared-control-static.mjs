...
for(const cls of ['.fl-btn','.fl-btn-primary','.fl-btn-secondary','.fl-btn-ghost','.fl-btn-danger','.fl-btn-icon','.fl-btn-nav','.fl-btn-locked','.fl-control-field','.fl-control-group']){
  assert(css.includes(cls),`missing shared control primitive ${cls}`);
}
...
assert(styles.indexOf('shared-control-system.css')>styles.indexOf('progressive-disclosure.css'),'shared control hierarchy should layer after previous Item 13 visual systems');
assert.equal(scripts.at(-1),'shared-control-system.js','shared control adapter should run last after all render layers');
...
assert(sw.includes("frontier-lab-v15"),'Item 13.6 should advance the offline cache to v15');

console.log(JSON.stringify({sharedControlStatic:'pass',variants:7,cache:'frontier-lab-v15'},null,2));
