...
// Live viewport transition: resizing in-place must update mode and add/remove table containment without reload.
{
  ...
  const wrap=page.locator('#responsive-resize-fixture .fl-responsive-table-wrap');assert.equal(await wrap.count(),0,'desktop-sized table should keep its original DOM when it fits');assert.equal(await page.locator('#responsive-resize-fixture > table').count(),1,'fitting desktop table should remain a direct child');

  await page.setViewportSize({width:390,height:844});await settle(page,120);
  assert.equal(await page.evaluate(()=>document.documentElement.dataset.flResponsiveMode),'phone-portrait','resize listener did not switch to phone portrait');assert.equal(await wrap.count(),1,'table was not wrapped after the viewport shrank');assert.equal(await wrap.getAttribute('data-fl-responsive-scrollable'),'true','table did not become locally scrollable after shrink');assert.equal(await wrap.getAttribute('role'),'region','shrunken table did not gain scroll-region semantics');

  await page.setViewportSize({width:1440,height:1000});await settle(page,120);
  assert.equal(await page.evaluate(()=>document.documentElement.dataset.flResponsiveMode),'desktop','resize listener did not return to desktop');assert.equal(await wrap.count(),0,'table wrapper was not removed when desktop space returned');assert.equal(await page.locator('#responsive-resize-fixture > table').count(),1,'table was not restored to original DOM after expansion');
