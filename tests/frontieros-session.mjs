...
try{
  ...
  const restoredData=await phase('phone-reload-session-restore',async()=>{await p.reload({waitUntil:'domcontentloaded'});await p.locator('.frontieros-mobile-appbar').waitFor({state:'visible',timeout:8000});const restored=await p.evaluate(()=>frontierOsSessionSnapshot());const mobileRestored=await p.evaluate(()=>frontierMobileShellSnapshot());assert.equal(restored.current.appId,beforeReload.current.appId,'session current app metadata did not survive reload');assert.equal(restored.current.detail,'run-1842','session detail did not survive reload');assert.equal(mobileRestored.currentApp,'training','reload did not visually restore Run Monitor');assert.equal(restored.history.length,beforeReload.history.length,'history did not survive reload');assert(restored.recents.some(x=>x.appId==='training'),'recent apps did not survive reload');assert.equal(restored.notifications.length,beforeReload.notifications.length,'notification state did not survive reload');assert.equal(restored.unread,0,'notification read state did not survive reload');return{restored,mobileRestored}},15000);
  const {restored,mobileRestored}=restoredData;
  await phase('phone-restored-evidence',async()=>{await p.evaluate(()=>frontierOsOpenNotificationCenter());await p.screenshot({path:path.join(out,'phone-restored-session.png'),fullPage:true});await p.evaluate(()=>frontierOsCloseNotificationCenter());const events=await p.evaluate(()=>frontierEventJournal({type:'os.*',limit:250}));assert(events.some(e=>e.type==='os.session.restored'),'session restore event missing');assert(events.some(e=>e.type==='os.mobile.app.opened'&&e.data.appId==='training'&&e.data.detail==='run-1842'),'restored mobile app-open event missing');assert(events.some(e=>e.type==='os.session.current-restored'&&e.data.appId==='training'&&e.data.detail==='run-1842'),'current-app restore event/detail missing')});
  ...
  const report={version:3,item:'P5.1.4',status:'pass',generatedAt:new Date().toISOString(),durationMs:Date.now()-startedAt,phoneHistory:restored.history.length,phoneRecents:restored.recents.length,phoneNotifications:restored.notifications.length,restoredApp:mobileRestored.currentApp,restoredDetail:restored.current.detail,desktopCurrent:ds.current.appId,pageErrors:errors.length,phases:progress.completed,evidence:['phone-notification-center.png','phone-restored-session.png','desktop-notification-center.png','phone-trace.zip','progress.json']};
  fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2)+'\n');fs.writeFileSync(path.join(out,'REPORT.md'),`# P5.1.4 Navigation, Notifications & Session\n\n- Status: **PASS**\n- Duration: **${report.durationMs} ms**\n- Instrumented phases: **${report.phases.length}**\n- Phone back/forward history: **PASS**\n- Forward-stack preservation: **PASS**\n- Notification deep link: **PASS**\n- Notification read state: **PASS**\n- Reload app restoration: **${report.restoredApp}/${report.restoredDetail}**\n- Recent apps: **PASS**\n- Desktop shell-aware navigation: **PASS**\n- Runtime page errors: **0**\n- Playwright trace: **captured**\n- Progress checkpoints: **captured**\n`);progress.status='pass';progress.phase='complete';writeProgress();console.log('P5.1.4 FrontierOS session regression passed');
}catch(error){
