...
async function dismissTransient(page){
  for(let i=0;i<12;i++){
    const story=page.locator('.story-overlay');if(await story.count()){const b=story.locator('button').last();if(await b.count()){await b.click();await settle(page);continue}}
    const milestone=page.locator('.feel-milestone');if(await milestone.count()){const b=milestone.locator('button').last();if(await b.count()){await b.click();await settle(page);continue}}
    break;
  }
}
...
for(const d of devices){
  ...
  const found=page.getByRole('button',{name:/found the lab/i});assert(await found.count(),`${d.name}: founder CTA missing`);await found.click();await settle(page);await dismissTransient(page);
  ...
  await page.evaluate(()=>{state.campaign ||= {version:1};state.campaign.graduated=true;state.campaign.companyPriority='research';state.campaign.modelReviewed=true;state.view='company';save();render()});await settle(page);await dismissTransient(page);await page.evaluate(()=>window.frontierCompanyDashboardSync?.());await settle(page);
  ...
  assert(groups.has('model'),`${d.name}: Model & Engineering group missing`);assert(groups.has('operations'),`${d.name}: Operations & Releases group missing`);assert(groups.size>=3,`${d.name}: expected at least three populated dashboard groups, got ${JSON.stringify([...groups])}`);
  let keyFound=0;for(const [selector,expectedGroup] of KEY_LAUNCHERS){const l=page.locator(selector).first();if(!(await l.count()))continue;keyFound++;assert(await l.evaluate((el,g)=>el.closest('.company-system-group')?.dataset.dashboardGroup===g,expectedGroup),`${d.name}: ${selector} not grouped under ${expectedGroup}`)}assert(keyFound>=7,`${d.name}: expected at least 7 known dashboard launchers, found ${keyFound}`);
  ...
  if(geometry.length)for(const lefts of geometry){const columns=new Set(lefts).size;if(d.name==='desktop')assert(columns>=2,`desktop: launcher grid should use multiple columns: ${JSON.stringify(lefts)}`);else assert.equal(columns,1,`mobile: launcher grid should collapse to one column: ${JSON.stringify(lefts)}`)}

  await page.evaluate(()=>{const shell=document.querySelector('.game-shell');const b=document.createElement('button');b.className='future-systems-launch';b.dataset.dashboardId='future-systems';b.innerHTML='<span>FUTURE SYSTEM</span><b>Quantum Planning</b><small>Future module fallback</small>';b.onclick=()=>{};shell.insertBefore(b,shell.querySelector('.world-grid'))});await settle(page);await page.evaluate(()=>window.frontierCompanyDashboardSync?.());await settle(page);
  const future=page.locator('.future-systems-launch');assert(await future.count(),`${d.name}: future launcher missing`);assert(await future.evaluate(el=>el.closest('.company-system-group')?.dataset.dashboardGroup==='other'),`${d.name}: future launcher should fall back to Other Lab Systems`);assert.equal(await page.locator('.company-system-hub').count(),1,`${d.name}: future launcher created duplicate hub`);
  const before=await page.locator('.company-system-hub button').count();await page.evaluate(()=>{window.frontierCompanyDashboardSync?.();window.frontierCompanyDashboardSync?.()});await settle(page);assert.equal(await page.locator('.company-system-hub button').count(),before,`${d.name}: repeated dashboard sync changed launcher count`);assert.equal(await page.locator('.company-system-hub').count(),1,`${d.name}: repeated dashboard sync duplicated hub`);

  const data=page.locator('.data-evals-launch').first();
  if(await data.count()){
    await dismissTransient(page);await data.click();await settle(page);
    assert.equal(await page.evaluate(()=>state.view),'dataEvals',`${d.name}: Data + Evals launcher lost navigation behavior after grouping`);
    await page.evaluate(()=>window.gameplayGoHome());await settle(page);await dismissTransient(page);await page.evaluate(()=>window.frontierCompanyDashboardSync?.());await settle(page);
