...
async function prepare(page){
  ...
  await page.evaluate(()=>window.frontierLockedStateSync?.());await settle(page,30);
}
...
for(const d of devices){
  ...
  await assertPlan(page,'dataEvalsOpen','firstCall',true);
  await assertPlan(page,'techDebtOpen','recover',false);
  const opsPlan=await assertPlan(page,'opsOpen','firstHire',false);assert.equal(opsPlan.unlockChapter,6,`${d.name}: Operations should identify Chapter 6`);
  await assertPlan(page,'releaseGovOpen','tradeoff',false);
  await assertPlan(page,'governanceOpen','graduated',false);
  await assertPlan(page,'models','firstModel',false);
  await assertPlan(page,'team','firstHire',false);

  const team=page.locator('.gameplay-bottom-nav button[data-campaign-target="team"]');
  const models=page.locator('.gameplay-bottom-nav button[data-campaign-target="models"]');
  const home=page.locator('.gameplay-bottom-nav button[data-campaign-target="home"]');
  assert.equal(await team.getAttribute('aria-disabled'),'true',`${d.name}: Team should expose locked semantics`);
  assert.equal(await models.getAttribute('aria-disabled'),'true',`${d.name}: Models should expose locked semantics`);
  assert(!(await home.evaluate(el=>el.classList.contains('fl-campaign-locked'))),`${d.name}: Home must remain unlocked`);
  assert(await team.locator('.fl-lock-meta').count(),`${d.name}: Team lock chapter badge missing`);
  ...
  const opsLaunch=companyLaunches.filter({hasText:/OPERATIONS/i}).first();assert(await opsLaunch.count(),`${d.name}: Operations launcher missing`);assert(await opsLaunch.evaluate(el=>el.classList.contains('fl-campaign-locked')),`${d.name}: Operations launcher should be locked`);
  assert((await opsLaunch.getAttribute('title')||'').includes('CHAPTER 6'),`${d.name}: Operations launcher title should explain unlock chapter`);
  ...
  const moreData=page.locator('.gameplay-system-grid button[data-campaign-target="dataEvalsOpen"]');assert(!(await moreData.evaluate(el=>el.classList.contains('fl-campaign-locked'))),`${d.name}: More-sheet Data + Evals should be unlocked`);
  await page.evaluate(()=>gameplayCloseMenu());

  // A locked launcher explains the prerequisite instead of navigating into the unavailable page.
  await opsLaunch.click();await settle(page,70);
  assert.equal(await page.evaluate(()=>state.view),'company',`${d.name}: locked Operations launcher changed view`);
  const panel=page.locator('.fl-lock-explainer');assert.equal(await panel.count(),1,`${d.name}: lock explainer missing`);
  assert.equal(await panel.getAttribute('role'),'region',`${d.name}: lock explainer region semantics missing`);
  assert(/Operations is not available yet/i.test(await panel.locator('h3').textContent()||''),`${d.name}: lock explainer target title missing`);
  const copy=(await panel.textContent())||'';assert(copy.includes('CHAPTER 1 · FIRST CALL'),`${d.name}: current prerequisite missing`);assert(copy.includes('CHAPTER 6 · BUILD THE LAB'),`${d.name}: unlock point missing`);assert(copy.includes('Campaign progress toward unlock'),`${d.name}: progress explanation missing`);
  assert(await panel.getByRole('button',{name:/Choose a training run/i}).count(),`${d.name}: current-objective CTA missing`);
  ...
  if(d.name==='mobile'){const h=await panel.getByRole('button',{name:/Choose a training run/i}).evaluate(el=>el.getBoundingClientRect().height);assert(h>=43,`mobile lock CTA below ~44px (${h})`)}
  ...
  // Local disabled actions are visually unavailable without receiving a campaign prerequisite.
  ...
  const blocked=page.locator('#qaUnavailable');assert(await blocked.evaluate(el=>el.classList.contains('fl-unavailable-now')),`${d.name}: disabled action missing unavailable treatment`);assert.equal(await blocked.getAttribute('aria-disabled'),null,`${d.name}: native disabled action should not be relabeled as campaign lock`);assert((await blocked.getAttribute('title')||'').includes('current simulation state'),`${d.name}: disabled action explanation missing`);
  await page.evaluate(()=>{const b=document.getElementById('qaUnavailable');b.disabled=false;window.frontierLockedStateSync?.()});assert(!(await blocked.evaluate(el=>el.classList.contains('fl-unavailable-now'))),`${d.name}: unavailable treatment survived re-enable`);

  // Graduation removes campaign locks and their owned aria/title metadata.
  await page.evaluate(()=>{state.campaign.graduated=true;state.campaign.companyPriority='Research velocity';save();render()});await settle(page,140);await dismissStory(page);await page.evaluate(()=>window.frontierLockedStateSync?.());await settle(page,30);
  assert.equal(await page.evaluate(()=>campaignCurrentStage().id),'graduated',`${d.name}: graduation fixture failed`);
  assert.equal((await page.evaluate(()=>campaignUnlockPlan('opsOpen').unlocked)),true,`${d.name}: Operations did not unlock after graduation`);
