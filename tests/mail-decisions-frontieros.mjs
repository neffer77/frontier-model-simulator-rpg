...
async function assertLayout(page,label){
  ...
  assert.equal(layout.controls.length,6,label+': missing decision controls');
...
async function run(surface,full){
  ...
  try{
    ...
    for(const action of ['approve','reject','delegate','follow-up','attach-evidence'])assert(await page.locator('[data-fm-decision="'+action+'"]').isDisabled());
    ...
    assert.equal((await page.evaluate(()=>frontierOsSessionSnapshot())).current?.detail,'thread/'+before.thread.id+'/from/inbox',label+': window interaction erased the persisted thread route');
    await page.reload({waitUntil:'domcontentloaded'});
