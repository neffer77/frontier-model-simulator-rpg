...
// Real empty Critical Path state: the secondary section should become one compact intentional row.
...
await page.evaluate(()=>render());await settle(page);
...
// Synthetic states verify future dense modules inherit the same rules and stable title/class keys.
await page.evaluate(()=>{
  state.view='criticalPath';
  document.getElementById('app').innerHTML=`<div id="pd-fixture">
    <section class="overview-card"><h2>Overview</h2><p>Primary context always visible.</p></section>
    <section class="alpha-card"><h2>Alpha work</h2><p>Ready work.</p></section>
    <section class="beta-card locked"><h2>Beta work</h2><p>Requires an unlock.</p></section>
    <section class="gamma-card"><h2>Gamma work</h2><p>No active projects.</p></section>
  </div>`;
  frontierDisclosureSync();
});await settle(page);
...
await beta.locator(':scope > .pd-toggle').click();await settle(page);
const betaKey=await beta.getAttribute('data-pd-key');
...
await page.evaluate(()=>{for(let i=0;i<5;i++)frontierDisclosureSync()});await settle(page);
assert.equal(await page.locator('#pd-fixture .pd-toggle').count(),3,'repeated disclosure synchronization created duplicate toggles');

// Switching out of the mobile breakpoint must actively strip disclosure chrome/classes.
await page.setViewportSize({width:1000,height:800});await settle(page);
assert.equal(await page.locator('#app .pd-toggle').count(),0,'desktop resize left mobile disclosure toggles behind');
assert.equal(await page.locator('#app .pd-enhanced').count(),0,'desktop resize left mobile disclosure classes behind');
await page.evaluate(()=>criticalPathOpen());await settle(page);
...
console.log('Progressive disclosure browser regression passed');
