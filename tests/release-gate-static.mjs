...
const blockerIds=['static-contracts','production-build','runtime-identity','command-event-bus','debug-bundle','frontieros-app-registry','browser-smoke','balance-pacing','technical-realism','replayability','browser-firewall','shared-surfaces','progressive-disclosure','shared-controls','company-dashboard','page-visual-sweep','empty-states','locked-states','overlay-system','accessibility','responsive','npc-advice-workstation','route-crawl','release-candidate','screenshot-regression'];
...
assert.equal(gateById.get('frontieros-app-registry').script,'test:app-registry','FrontierOS app registry must remain release blocking');
assert.equal(gateById.get('frontieros-app-registry').evidence,'artifacts/app-registry/report.json','app registry evidence path drifted');
