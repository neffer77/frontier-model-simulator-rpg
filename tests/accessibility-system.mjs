...
async function clearStory(page){if(await page.locator('.story-overlay').count()){await page.keyboard.press('Escape');await settle(page,100);await a11ySync(page)}}
...
for(const d of devices){
  ...
  const story=page.locator('[data-fl-overlay-panel="story"]');assert.equal(await story.count(),1,`${d.name}: intro story missing`);
  assert.equal(await story.getAttribute('role'),'dialog',`${d.name}: story lost dialog role`);
  const storyProgress=story.locator('[role="progressbar"]');assert.equal(await storyProgress.count(),1,`${d.name}: story progressbar semantics missing`);
  assert.equal(await storyProgress.getAttribute('aria-valuemax'),'100',`${d.name}: story progress max missing`);
  await auditClean(page,`${d.name}/story`);await clearStory(page);

  // Company shell: heading, selected role, campaign progress and legacy muted-color bridge.
  assert(await page.locator('#app h1').count(),`${d.name}: current workspace has no h1`);
  const rolebar=page.locator('.rolebar');assert.equal(await rolebar.getAttribute('aria-label'),'Engineering role',`${d.name}: role navigation unlabeled`);
  assert.equal(await rolebar.locator('button[aria-pressed="true"]').count(),1,`${d.name}: active engineering role semantics missing`);
  const campaign=page.locator('.campaign-dots');assert.equal(await campaign.getAttribute('role'),'progressbar',`${d.name}: campaign progress semantics missing`);
  assert.equal(await campaign.getAttribute('aria-valuemax'),'7',`${d.name}: campaign progress max incorrect`);
  const colorBridge=await page.evaluate(()=>{
    const sample=document.querySelector('.sub'),probe=document.createElement('span');probe.style.color='var(--fl-text-muted)';document.body.appendChild(probe);const a=sample?getComputedStyle(sample).color:null,b=getComputedStyle(probe).color;probe.remove();return {actual:a,expected:b};
  });assert.equal(colorBridge.actual,colorBridge.expected,`${d.name}: legacy muted copy did not use accessible semantic color`);
  await auditClean(page,`${d.name}/company`);

  // Keyboard modality produces a clearly visible focus ring.
  ...
  assert.equal(await page.evaluate(()=>document.activeElement?.classList.contains('fl-skip-link')),true,`${d.name}: first keyboard stop should be skip link`);
  ...
  // More-sheet symbol close is named; campaign locks remain readable and semantic.
  await page.evaluate(()=>gameplayToggleMenu());await settle(page,80);await a11ySync(page);
  const moreClose=page.locator('.gameplay-more-sheet header button');assert.equal(await moreClose.getAttribute('aria-label'),'Close',`${d.name}: More close button lacks accessible name`);
  const locked=page.locator('.gameplay-system-grid button[aria-disabled="true"]');assert(await locked.count()>0,`${d.name}: expected early campaign locks`);await auditClean(page,`${d.name}/more`);await page.evaluate(()=>gameplayCloseMenu());await settle(page,60);

  // Technical explainer: named close control and explicit new-tab announcement.
  await page.evaluate(()=>showExplain('FSDP'));await settle(page,70);await a11ySync(page);
  const modal=page.locator('[data-fl-overlay-panel="modal"]');assert.equal(await modal.locator('.x').getAttribute('aria-label'),'Close',`${d.name}: explainer close label missing`);
  const source=modal.locator('a[target="_blank"]');assert(/opens in new tab/i.test(await source.getAttribute('aria-label')||''),`${d.name}: external-source link does not announce new tab`);await auditClean(page,`${d.name}/modal`);await page.keyboard.press('Escape');await settle(page,60);
  ...
  // Future legacy markup receives safe derived names/scopes without feature-specific code.
  ...
  if(d.name==='mobile'){
    // Ordinary touch controls are at least ~44 CSS px; inline term/help affordances use the compact 32px exception.
    ...
    assert.deepEqual(undersized,[],`${d.name}: undersized touch targets ${JSON.stringify(undersized)}`);

    // Increasing the root text size makes rem-backed microcopy grow and must not create page-level horizontal overflow.
