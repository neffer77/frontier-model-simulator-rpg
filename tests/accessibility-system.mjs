...
for(const d of devices){
  ...
  // The canonical live incident is the engineering workstation, not the retired
  // Metrics / Systems / Data quiz. Validate its alert semantics and tool navigation.
  await page.evaluate(()=>{state.story.seen=[...(state.story?.seen||[]).filter(x=>x!=='firstIncident'),'firstIncident'];state.story.active=null;openIncident('nan')});await settle(page,120);await a11ySync(page);
  ...
  assert(await incident.locator('.workstation,.ws-tools').count(),`${d.name}: engineering workstation missing`);
  const tablist=incident.locator('.ws-tools[role="tablist"]');assert.equal(await tablist.count(),1,`${d.name}: workstation tool tablist semantics missing`);
  const tabs=tablist.locator('[role="tab"]');assert.equal(await tabs.count(),8,`${d.name}: workstation should expose eight diagnostic tool tabs`);const selected=tablist.locator('[role="tab"][aria-selected="true"]');assert.equal(await selected.count(),1,`${d.name}: exactly one workstation tool must be selected`);
  await selected.focus();await page.keyboard.press('ArrowDown');await settle(page,100);await a11ySync(page);assert(/profiler/i.test((await tablist.locator('[role="tab"][aria-selected="true"]').textContent())||''),`${d.name}: ArrowDown did not activate next diagnostic tool`);
  assert.equal(await incident.locator('.ws-main').getAttribute('role'),'tabpanel',`${d.name}: diagnostic result surface is not a tabpanel`);await auditClean(page,`${d.name}/incident`);
