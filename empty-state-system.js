// Item 13.9 — state-aware empty / zero-data guidance for simulator modules.
(function(){
  let queued=false;
  const viewToPage={modelLab:'model-lab',portfolio:'portfolio',criticalPath:'critical-path',operations:'operations',hiring:'hiring',governance:'governance',program:'programs',postmortems:'postmortems'};
  const safeState=()=>{try{return typeof state!=='undefined'?state:null}catch{return null}};
  const byHeading=(root,selector,text)=>[...root.querySelectorAll(selector)].find(el=>el.querySelector('h2,h3')?.textContent.trim()===text)||null;
  const firstText=(root,selector,pattern)=>[...root.querySelectorAll(selector)].find(el=>pattern.test((el.textContent||'').trim()))||null;
