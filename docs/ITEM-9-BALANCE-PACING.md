...
### Funding pacing
The seed milestone moves to reputation 2 so shipping the first small model stabilizes the company rather than forcing the player to finish the guided campaign with an immediate cash cliff. Later funding thresholds remain progressively harder.
...
## Playtest telemetry
`balanceReport()` returns a local playthrough snapshot including resources, milestone days, target/fast/slow pacing classification, campaign stage, debt counts, and action statistics.
