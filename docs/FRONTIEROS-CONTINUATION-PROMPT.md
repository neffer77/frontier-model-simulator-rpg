...
- Post-merge Cross-device run `34498423700`, attempt 1, failed in the technical-realism test's fixed-delay story click loop. Mail triage passed. Pages run `34498423693` passed its complete release gate and live verification on the same merge SHA. The `fix/realism-story-startup` branch replaces the shared balance/realism startup race with a real Skip click and waits for canonical story closure plus DOM removal. Its regression covers delayed animation frames, persisted dismissal, unchanged resources, and rejection when Skip does not close the story. Verify its PR and the main QA rerun before starting the next feature; the fix changes test setup only.
