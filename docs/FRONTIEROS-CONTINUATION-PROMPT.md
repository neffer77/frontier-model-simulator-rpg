# FrontierOS continuation prompt — P5.3.6 Mail reviewer context
...
- P5.3.6 on `feature/frontieros-mail-reviewer-context` starts from that merged main. It adds canonical pending/delegated row labels and the current eligible reviewer's name to the existing Needs decision view. Missing reviewers remain explicit and actionable; historical names and money stay unchanged. The domain regression failed on the base before implementation. Existing domain/browser triage gates now cover reassignment, live names, unavailable reviewers, escaped long-name wrapping, projection recovery and reload. See `docs/P5.3.6-MAIL-REVIEWER-CONTEXT.md` and the PR for actual current-head Actions results. Mail v3, committee v2, policy v19, 45 blockers, 15 apps, 190 routes and 255 captures are preserved; cache is v56. Verify this slice's merge status before starting another.
...
## Next single slice after P5.3.6

After P5.3.6 is validated and merged, preserve **the originating Mail folder across a Finance or Artifacts reload before returning to the exact request**. Confirm the gap with actual Needs decision → request → Finance/Artifacts → reload → Back to request → message list clicks. Mail's own folder/thread routes already survive reload and ordinary cross-app return already preserves the exact thread. Extend the existing navigation owners to carry the original folder through the cross-app route, retaining old plain-thread routes. Cover archived actionable requests, closed requests, reload and unchanged money/history. Do not add a new navigation store or rebuild the implemented triage/reviewer UI.
