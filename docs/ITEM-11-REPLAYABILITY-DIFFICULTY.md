...
A fresh player defaults to **Standard + Balanced Lab + Frontier Generalist**. That combination preserves the canonical Item 9 economy while still giving the player a visible run objective. Specialized archetypes become optional replay choices rather than silently replacing the canonical simulation.
...
| Frontier | 90% | 90% | 90% | 90% | +1 day / -$0.08M beyond base | 1.35× |
| Redline | 75% | 75% | 80% | 80% | +2 days / -$0.18M beyond base | 1.75× |
...
Funding messages are rewritten after difficulty scaling so the company history reports the resources the player actually received rather than the unscaled canonical round amount.

## Lab archetypes

### Balanced Lab — canonical default
- 100% starting cash
- 100% starting compute
- no starting insight/reputation modifier
- no special economic reward

This is the neutral Item 9 economy and the default for a first playthrough.
...
### Frontier Generalist — first-run default
Before the deadline:
...
- 4 difficulty modes
- 5 archetypes, including the neutral canonical Balanced Lab
- 4 challenges

That creates **80 core run configurations** before company priorities, hiring, model lineage, incidents, financing, technical debt, architecture choices, and other simulator systems cause further divergence.

## Realism boundary

Difficulty, archetype bonuses, challenge deadlines, medals, career points, and New Game+ perks are explicitly registered in the Item 10 Realism Audit as **game abstractions**. They are not claims about real frontier-lab productivity, failure rates, financing, or career progression.

The technical layer remains invariant across modes. For example, changing from Standard to Redline does not change the 6ND teaching approximation, sparse-MoE active-parameter accounting, incident evidence, or which diagnosis is technically correct.
...
- a fresh save defaults to Standard + Balanced + Frontier Generalist
- the neutral archetype preserves 1× cash/compute and zero bonus insight/reputation
...
- Redline seed funding grants exactly 80% of canonical cash/compute
- the funding log reports the scaled payout instead of the canonical amount
- a Redline wrong diagnosis adds the configured time/cash consequence while preserving canonical telemetry
- challenge deadline scaling works
- the Run Archive opens and reports the 80-combination matrix
...
- a technically completed objective after its deadline does not become a clear
